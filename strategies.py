...
from graph_io import write_seed_node_file
import time
...
def community_control(g, N, alpha=1, algorithm="degree"):
    G = nx.Graph(g)
    comms = nx.community.greedy_modularity_communities(G)
    max_comm = list(comms[0])
    H = G.subgraph(max_comm)
    centralities = centrality(H, algorithm=algorithm)
    measures = [centralities[u] for u in H.nodes()]
